...
namespace Config {
	...
	const vector<array<string, 2>> descriptions = {
		...
		{"shown_boxes", 		"#* Manually set which boxes to show. Available values are \"cpu mem net proc agent\" and \"gpu0\" through \"gpu5\", separate values with whitespace."},
	...
	std::unordered_map<std::string_view, string> strings = {
		{"color_theme", "Default"},
		{"shown_boxes", "cpu mem net proc agent"},
